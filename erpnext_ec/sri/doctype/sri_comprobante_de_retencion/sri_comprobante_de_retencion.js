...
//NO USAR AQUI
//SetListSriButtons('SRI Comprobante de Retencion');
...
frappe.ui.form.on('SRI Comprobante de Retencion', 
{
	...
	refresh(frm) {
        ...
        SetFormSriButtons(frm, 'SRI Comprobante de Retencion');
	},
    ...
    onload: function(frm) 
    {
        setTimeout(
            async function () 
            {
				...
				// 	args: 
				// 	{
				// 		doctype:"SRI Comprobante de Retencion",
...
frappe.ui.form.on("Purchase Taxes and Charges Ec", "codDocSustentoLink", function(frm, cdt, cdn) {     
    ...
    //let type_document_sri = frappe.get_doc('SRI Tipo de Comprobante', item.codDocSustentoLink);
    //console.log(type_document_sri);    
    
    setTimeout(
        async function () 
        {
            var type_document_sri = await frappe.db.get_doc('SRI Tipo de Comprobante', item.codDocSustentoLink);
            console.log(type_document_sri);
